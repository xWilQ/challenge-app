...
export default function Home() {
  ...
  const handleSkip = async (challenge) => {
    try {
      ...
      const newChallenge = (await getDocs(collection(db, 'kalajoki24'))).docs
  ...
  const handleSubmit = async (challenge, comment, photoStatus) => {
    try {
      ...
      // Add bonus challenge after user completes 5 challenges.
      if (ammountCompleted == 4){

        const bonusChallenge = {
          id: 'BONUS#CHALLENGE',
          title: 'VIHASPAUHAN MERIROSVO AARRE',
          description: 'Tämä on ryhmätehtävä, joten poikkeuksellisesti tästä haasteesta saa keskustella toisten kanssa, mutta huomioi että kaikilla ei välttämättä ole tätä haastetta. Wanhan Wiljamin kalaWerkkoon oli takertunut pulloposti viesti: Polku aarteelle on haastava ja se kulkee solan kautta. Ennen aarretta on ohitettava jyrkkä seinämä.',
        }
